...
import { parseAudienceSheet, parseCrossPrefSheet, inferGrade } from '../live-schedule-dashboard/src/utils/parser'
...
import type { LiveStream, AudienceSegment, AssignedAudience, CrossCategoryPref } from '../live-schedule-dashboard/src/types'

// ====== 1. Load Feishu config ======
...
async function getTenantToken() {
  ...
  if (!data.tenant_access_token) throw new Error(`Token failed: ${data.msg}`)
  return data.tenant_access_token
}

async function listRecords(token: string, appToken: string, tableId: string) {
  const records: any[] = []
  let pageToken = ''
  while (true) {
...
async function updateRecord(token: string, appToken: string, tableId: string, recordId: string, fields: any) {
  const res = await fetch(`https://open.feishu.cn/open-apis/bitable/v1/apps/${appToken}/tables/${tableId}/records/${recordId}`, {
    method: 'PUT',
...
// ====== 2. Parse Audience Inventory (reference 5.18-24) ======
const audiencePath = path.join(__dirname, '../5月18-24排期人数.xlsx')
const audienceBuf = fs.readFileSync(audiencePath)
const audienceSegments = parseAudienceSheet(audienceBuf)
console.log('Audience segments:', audienceSegments.length)

// ====== 3. Parse Cross Preferences ======
...
if (fs.existsSync(crossPrefPath)) {
  const crossBuf = fs.readFileSync(crossPrefPath)
  const parsed = parseCrossPrefSheet(crossBuf)
  crossCategoryPrefs = parsed.crossCategoryPrefs
  console.log('Cross prefs:', crossCategoryPrefs.length)
}

// ====== 4. Read Feishu records and build lives ======
async function buildLivesFromFeishu() {
  const token = await getTenantToken()
  const records = await listRecords(token, config.appToken, config.tableId)
  // Filter for 5.25-5.31 (date field is timestamp in ms)
  const weekStart = new Date('2026-05-25').getTime()
  const weekEnd = new Date('2026-05-31').getTime()
  ...
  const lives: LiveStream[] = weekRecords.map((r, idx) => {
    const f = r.fields
    ...
    // Extract categories for joint live
    ...
    return {
      id: `live-${idx}`,
      name,
      startTime: slot === 'morning' ? '07:00' : '19:00',
      date: dateStr,
      type: slot.startsWith('fake') ? 'fake' : 'real',
      category: normalizeCategory(f.category || name),
      line,
      slot: slot as any,
      grade: grade as any,
      owner: f.owner || '',
  ...
  return { lives, token, records: weekRecords }
}

// ====== 5. AutoSchedule Core (from test-autoSchedule.ts) ======
...
function checkConflicts(live: LiveStream, seg: AudienceSegment, allLives: LiveStream[], historyRecords: any[]): string[] {
...
function tryAssign(live: LiveStream, seg: AudienceSegment, allSegments: AudienceSegment[], maxCount?: number, allowReuse: boolean = false): AudienceSegment | null {
  ...
  if (!allowReuse && seg.assignedTo && seg.assignedTo !== live.id) {
    // skip transfer for simplicity in this script
  }
...
// ====== 6. Main ======
async function main() {
  const { lives, token, records } = await buildLivesFromFeishu()
  console.log('Lives to schedule:', lives.length)
  lives.forEach(l => console.log(l.date, l.slot, l.name, l.grade, l.line, 'joint=' + l.isJoint))
  ...
  const result = runAutoSchedule(lives, testSegments)

  // Print results
  console.log('\n=== 排期结果 ===\n')
  let totalExposure = 0
  for (const { live } of result.scored) {
    totalExposure += live.exposure
    const target = getTarget(live)
    console.log(`${live.date} ${live.slot} ${live.name} (等级:${live.grade}) 目标:${target.toLocaleString()} 实际:${live.exposure.toLocaleString()} 段数:${live.assignedAudiences.length}`)
  ...
  // === Post-adjust: ensure 健康营养 >= 500K ===
  const jkLive = lives.find(l => l.name === '健康营养')
  const jjLive = lives.find(l => l.name === '君合太极晨练')
  const jgLive = lives.find(l => l.name === '【伪直播】居家古法')
  if (jkLive && jjLive && jgLive) {
    // Transfer 健康营养(58,391) from 君合太极晨练
    const jjIdx = jjLive.assignedAudiences.findIndex(a => a.category === '健康营养' && a.timeRange === '2026年1月26日—2026年5月10日')
    if (jjIdx >= 0) {
      const seg = jjLive.assignedAudiences.splice(jjIdx, 1)[0]
      jjLive.exposure -= seg.count
      jkLive.assignedAudiences.push(seg)
      jkLive.exposure += seg.count
      console.log(`Transferred 健康营养(58,391) from 君合太极晨练 to 健康营养`)
    }
    // Transfer 健康营养(33,652) from 居家古法
    const jgIdx = jgLive.assignedAudiences.findIndex(a => a.category === '健康营养' && a.timeRange === '2025年4月28日—2025年10月26日')
    if (jgIdx >= 0) {
      const seg = jgLive.assignedAudiences.splice(jgIdx, 1)[0]
      jgLive.exposure -= seg.count
      jkLive.assignedAudiences.push(seg)
      jkLive.exposure += seg.count
      console.log(`Transferred 健康营养(33,652) from 居家古法 to 健康营养`)
    }
    console.log(`Post-adjust 健康营养 exposure=${jkLive.exposure.toLocaleString()}`)
  }
  ...
  for (let i = 0; i < lives.length; i++) {
    ...
    const healthAud = live.assignedAudiences.filter(a => a.line === 'health')
    const beautyAud = live.assignedAudiences.filter(a => a.line === 'beauty')
    const interestAud = live.assignedAudiences.filter(a => a.line === 'interest')

    const formatAud = (auds: AssignedAudience[]) => auds.map(a => `${a.category}（${a.count}）${a.timeRange}`).join('\n')

    await updateRecord(token, config.appToken, config.tableId, record.record_id, {
      exposure: live.exposure,
      healthAudience: formatAud(healthAud),
      beautyAudience: formatAud(beautyAud),
      interestAudience: formatAud(interestAud),
    })
    console.log(`Updated: ${live.name} exposure=${live.exposure}`)
  }
