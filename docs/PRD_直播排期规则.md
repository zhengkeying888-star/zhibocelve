...
**核心变更（v3.1）**：
- 修复明细表数字解析：支持 `¥35,000`、`12,500` 等带货币符号/千分位逗号的格式。
...
5. 自动识别 `【上次直播排期】` 段落，将对应 audience 路由到伪直播历史记录。
...
- **兴趣线**：手机摄影、摄影美学、唱歌、短视频、风光摄影、相机摄影、声乐、国际声乐、电子琴、键盘乐、真书法、油画、国画、国学朗诵、戏曲、舞蹈、优雅舞蹈、茶道、编织工艺美学、钩针编织美学、美学收纳
...
1. 直播优先级排序（scored）
2. 伪直播预处理（全局剔除）
3. Round 1：严格分配（每段只用一次）
4. Round 2：复用填充（未达目标的直播可复用，3天间隔）
5. Round 3：强制填充（剩余段分配给最缺曝光的直播）
6. 验证与冲突检查
```
...
1. 遍历所有伪直播，将其 `assignedAudiences` 中的每个段标记为全局已使用。
2. 伪直播本身 `assignedAudiences = []`，`exposure = 0`。
...
| 2 | 状态可用 | `seg.status !== 'used'` 且未被伪直播全局剔除 |
...
- 循环直到没有直播能获得新的候选段。
- **目的**：优先保证每段 audience 只被使用一次，最大化覆盖不同存量池。

#### Round 2：复用填充

- 仅对 `exposure < target` 的直播允许复用。
- 候选条件放宽：`assignedDates.length === 1` 且 `daysBetween >= 3`。
- 按直播优先级遍历，继续分配直到没有符合条件的候选段。
- **目的**：让未达目标的直播通过复用补足曝光。

#### Round 3：强制填充

- 遍历所有剩余 `status === 'available'` 的段。
- 对每个剩余段，找出所有可接收的直播（满足线级、品类族、次数限制）。
- 按当前 exposure 升序排序，将段分配给最缺曝光的直播。
- **目的**：确保总库存被完全分配，不浪费任何 audience 段。
...
| **v3.1** | **2026-05-13** | **修复数字解析**：支持 `¥35,000`、`12,500` 等带货币符号/千分位逗号格式；**修复 localStorage fallback** 持久化 `categoryHistoricalStats`；**扩充直播排期规则章节**（细化早间/晚间/伪直播/朋友圈解析规则） |
