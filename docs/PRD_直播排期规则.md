...
5. **预估 GMV 降序**：`count × crossRate × LTV`
6. **count 降序**
...
| v2.1 | 2026-05-09 | **联合直播目标修正**：第一场完整目标 + 后续砍半（×0.5）；**完成版排期解析**：支持嵌入 audience 分配的多行解析、跳过 Excel 时间数字与资源位标注；**autoSchedule 校准**：同品类族优先、品类/timeRange 去重、超大段降权、GMV 排序、允许 130% 超额分配 |
...
1. **规则学习**：拖拽/手动调整后沉淀为 `learnedRules`，用于下次 autoSchedule 调整排序权重。
2. **完成版排期直接回写**：当前完成版排期可正确解析并显示，但尚未支持将手动调整后的结果导出为新的"正确版排期"格式。
